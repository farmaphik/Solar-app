...
class SolarAppV1_1:
    ...
    def exit(self, event=None):
        ...
        messagebox.showinfo(f"Аккаунт", f"Вы вышли из аккаунта! {login} ")
    ...
    def show_password(self, event=None):
        if not self.current_login:
            messagebox.showinfo("Аккаунт", "Вы ещё не авторизованы")
            return
        messagebox.showinfo("Данные аккаунта", f"Логин: {self.current_login}\nПароль: {self.current_password}")
    ...
    def login(self):
        ...
        with open("login.txt", "r", encoding="utf-8") as f:
            accounts = dict(line.strip().split(":", 1) for line in f if ":" in line)
    ...
    # -------------------- РАСЧЁТ СРЕДНЕЙ СТОИМОСТИ И Т.Д. --------------------
    ...
    def create_conv_tab(self):
        ...
        ttk.Button(frame, text="Шпоргалка", command=lambda: messagebox.showinfo("Шпоргалка", "USD-Доллар,EUR-Евро,RUB-Рубль,KZT-Тенге,UAH-Гривна,GBP-Фунт стерлингов,CNY-Юань,TRY-Турецкая лира,GEL-Лари,BYN-Белорусский_рубль,KGS-Сом,UZS-Сум,CHF-Франк,JPY-Иена")).pack(anchor="ne", padx=10, pady=5)
    ...
    def create_info_tab(self):
        ...
        info_text.insert(
            ...
            "1.0.22-Переработка кода,добавление пасхалки/выхода/шпоргалки из акаунта и исправление мелочей!"
    ...
    def create_help_tab(self):
        ...
        ttk.Button(frame,text="Нажми меня!", command=lambda: messagebox.showinfo("Пасхалка", "Ты крутой за это я тебе дам приз!, напиши в поддержку и скинь скрин этого!")).pack(pady=5)
